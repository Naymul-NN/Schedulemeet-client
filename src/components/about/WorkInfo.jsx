      ...
      <div className="workInfoWrapper grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3  gap-y-12 gap-x-4  w-[80%] m-auto  ">
        ...
        {infoData &&
          infoData.map((data) => (
            <div
              key={data?.id}
              className="infoCard  flex flex-col justify-start items-start gap-2   "
            >
              {/* card top  ,  number */}
              <h1 className=" text-5xl font-bold  pb-3 border-b-2  border-gray-200 text-[#33A30F]  ">
                {" "}
                {data?.number}{" "}
              </h1>
              {/* card top  */}
              ...
              {/* card bottom , info  */}
              <h1 className=" text-xl text-gray-50 ">{data?.description}</h1>
              {/* card bottom , info  */}
            </div>
          ))}
